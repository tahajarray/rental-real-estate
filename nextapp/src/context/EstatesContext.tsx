...
interface EstatesContextType {
  ...
  bookVisitSlot: (estateId: string, slotId: string, userName: string) => boolean;
  /** All booked visit slots across every property, most recent first — this is
   * what powers the admin "Recent Bookings" view, computed straight from the
...
// (a) admin edits and bookings survive a page refresh, and (b) other tabs on
...
export function EstatesProvider({ children }: { children: React.ReactNode }) {
  ...
  // what makes a booking made as a user show up live on the admin side.
  useEffect(() => {
  ...
  const bookVisitSlot = (estateId: string, slotId: string, userName: string): boolean => {
      ...
      e.id === estateId
        ? {
            ...
            visitSlots: e.visitSlots.map((s) =>
              s.id === slotId ? { ...s, isBooked: true, bookedByName: userName, bookedAt } : s
            ),
          }
        : e
    );
  ...
  // admin side always reflects exactly what users have booked — no separate
