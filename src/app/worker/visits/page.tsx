...
function WorkerVisitsPage() {
  const { estates, addVisitSlot, removeVisitSlot, approveBooking } = useEstates();
  ...
  const approveSlot = (slotId: string, bookedByUserId?: string) => {
    if (!selected) return;
    approveBooking(selected.id, slotId);
    if (bookedByUserId) {
      addNotification({
        userId: bookedByUserId,
        estateId: selected.id,
        estateTitle: selected.title,
        message: `Your visit to "${selected.title}" has been approved.`,
      });
    }
  };
                    ...
                    <div key={slot.id} className="flex items-center justify-between px-5 py-3.5">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                          !slot.isBooked ? "bg-[#ECFDF5] text-[#10B981]" :
                          slot.bookingStatus === "approved" ? "bg-[#EFF6FF] text-[#2563EB]" : "bg-[#FFF7ED] text-[#F59E0B]"
                        }`}>
                          {!slot.isBooked ? "Open" : slot.bookingStatus === "approved" ? "Approved" : "Pending"}
                        </span>
                        {slot.isBooked && slot.bookingStatus !== "approved" && (
                          <button
                            onClick={() => approveSlot(slot.id, slot.bookedByUserId)}
                            className="w-8 h-8 rounded-lg border border-[#E2E8F0] flex items-center justify-center text-[#10B981] hover:border-[#6EE7B7]"
                        ...
                        <button
                          ...
                          title={slot.isBooked ? "Delete booking" : "Delete slot"}
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
...
export default function worker_visits_Guard() {
  return (
    <WorkerRoute>
      <WorkerVisitsPage />
    </WorkerRoute>
