...
import { Plus, Edit, Trash2, Eye, Bookmark, BarChart3, Building2, CheckCircle, DollarSign, CalendarClock, CalendarCheck, UserRound, Users, Check } from "lucide-react";
...
function WorkerDashboard() {
  ...
  const { user } = useAuth();
  const { estates, deleteEstate, updateEstate, bookings, approveBooking, deleteBooking } = useEstates();
  const { addNotification } = useNotifications();
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const handleApprove = (b: (typeof bookings)[number]) => {
    approveBooking(b.estateId, b.slotId);
    if (b.userId) {
      addNotification({
        userId: b.userId,
        estateId: b.estateId,
        estateTitle: b.estateTitle,
        message: `Your visit to "${b.estateTitle}" on ${new Date(b.date).toLocaleDateString(undefined, { month: "short", day: "numeric" })} at ${b.startTime} has been approved.`,
      });
    }
  };

  const handleDeleteBooking = (b: (typeof bookings)[number]) => {
    deleteBooking(b.estateId, b.slotId);
  };

  const recordAudit = (action: "estate_deleted" | "estate_status_changed", estate: Estate, details?: string) => {
    if (!user) return;
    logAudit({
      actorId: user.id,
      actorName: user.name,
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
          <div>
            <h1 className="text-[#1F2937] font-bold" style={{ fontSize: "1.75rem" }}>Worker Dashboard</h1>
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => router.push("/worker/visits")}
              variant="outline"
              className="rounded-xl border-[#E2E8F0] flex items-center gap-2"
            >
            ...
            <Button
              onClick={() => router.push("/worker/add-estate")}
        ...
        {/* Recent Bookings — makes it immediately visible to the worker whenever
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden mb-8">
          ...
          {bookings.length === 0 ? (
            ...
            <div className="divide-y divide-[#F1F5F9] max-h-96 overflow-y-auto">
              {bookings.map((b) => (
                <div key={b.slotId} className="flex items-center gap-3 px-5 py-3.5 hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-[#1F2937] flex items-center gap-2 flex-wrap">
                      <span className="font-medium">{b.userName}</span> booked a visit
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        b.bookingStatus === "approved" ? "bg-[#ECFDF5] text-[#10B981]" : "bg-[#FFF7ED] text-[#F59E0B]"
                      }`}>
                        {b.bookingStatus === "approved" ? "Approved" : "Pending"}
                      </span>
                    </p>
                    <p className="text-xs text-[#64748B] truncate">{b.estateTitle}</p>
                    <p className="text-[#94A3B8] mt-0.5" style={{ fontSize: "0.7rem" }}>
                      {new Date(b.date).toLocaleDateString(undefined, { month: "short", day: "numeric" })} · {b.startTime} · {timeAgo(b.bookedAt)}
                    </p>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {b.bookingStatus !== "approved" && (
                      <Button
                        ...
                        className="bg-[#10B981] hover:bg-[#059669] text-white rounded-lg h-8 px-2.5 text-xs flex items-center gap-1"
                      >
        ...
        {estates.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#E2E8F0] p-16 text-center">
            ...
            <Button onClick={() => router.push("/worker/add-estate")} className="bg-[#2563EB] text-white rounded-xl">
                ...
                <div key={estate.id} className="flex items-center gap-4 px-5 py-4 hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        const newStatus = nextStatus(estate);
                        updateEstate(estate.id, { status: newStatus });
                        recordAudit("estate_status_changed", estate, `${estate.status} → ${newStatus}`);
                      }}
          ...
          <DialogContent className="rounded-2xl border border-[#E2E8F0]">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  if (deleteId) {
                    const estate = estates.find((e) => e.id === deleteId);
                    deleteEstate(deleteId);
                    if (estate) recordAudit("estate_deleted", estate);
                    setDeleteId(null);
                  }
                }}
...
export default function worker_Guard() {
  return (
    <WorkerRoute>
      <WorkerDashboard />
    </WorkerRoute>
